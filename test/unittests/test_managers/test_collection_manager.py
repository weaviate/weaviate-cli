...
def test_create_collection_failure(mock_client):
    ...
    # Test that it raises the expected exception
    with pytest.raises(Exception) as exc_info:
        manager.create_collection(collection="TestCollection", replication_factor=3, async_enabled=True, vector_index="hnsw", inverted_index=None, training_limit=10000, multitenant=False, auto_tenant_creation=False, auto_tenant_activation=False, auto_schema=False, shards=1, vectorizer=None)
    
    #Verify the error message
    assert "Error creating Collection 'TestCollection': Connection to Weaviate failed" in str(exc_info.value)
    # Verify exists was called but create was not
    mock_collections.exists.assert_called_once_with("TestCollection")
    mock_collections.create.assert_called_once()
...
def test_delete_all_collections(mock_client):
    ...
    mock_client.collections = mock_collections
    manager = CollectionManager(mock_client)
    mock_client.collections.list_all.return_value = ["Collection1", "Collection2", "Collection3"]
...
def test_update_collection(mock_client):
    ...
    assert mock_collection.config.update.call_args.kwargs['description'] == "Updated description"
    assert mock_collection.config.update.call_args.kwargs['replication_config'].factor == 3
    assert mock_collection.config.update.call_args.kwargs['multi_tenancy_config'].autoTenantCreation is True
    assert mock_collection.config.update.call_args.kwargs['multi_tenancy_config'].autoTenantActivation is True
...
def test_update_nonexistent_collection(mock_client):
    ...
    assert "Error: Collection 'TestCollection' does not exist in Weaviate." in str(exc_info.value)
